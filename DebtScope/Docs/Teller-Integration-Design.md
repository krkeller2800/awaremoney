# Teller API Integration Design for DebtScope

## Objective
Integrate the [Teller API](https://teller.io) into `DebtScope` to allow users to directly connect their bank accounts for real-time, synchronous fetching of balances and transactions. This will complement the existing manual statement import flow.
...
- **Sync Trigger:** Sync can be triggered manually via a "Pull to Refresh" on the Accounts list, or automatically on app foregrounding (using `scenePhase` in `DebtScopeApp`).

## 2. Data Mapping Strategy

### Accounts
Map the Teller Account object to `DebtScope`'s `Account` SwiftData model:
...
### Transactions
Map the Teller Transaction object to `DebtScope`'s `Transaction` SwiftData model:
- `id`: Generate a new `UUID`.
- `datePosted`: Parse from Teller's `date`.
- `amount`: Map from Teller's `amount`. Note: Teller amounts for debits are positive; we need to align this with `DebtScope`'s internal sign convention (usually expenses are negative, or handled via `Transaction.Kind`).
...
## 4. Security & Privacy
- `DebtScope` will only fetch data; no money movement capabilities (like Zelle) will be requested or implemented.
