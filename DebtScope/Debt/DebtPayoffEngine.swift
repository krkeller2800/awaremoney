...
public enum DebtPayoffEngine {
    ...
    public static func plan(
        ...
        // Determine a stable month-start keyed dictionary
        let cal = Calendar(identifier: .gregorian)
        var keyed: [Date: Decimal] = [:]
        for (d, v) in budgetByMonth {
            let comps = cal.dateComponents([.year, .month], from: d)
            if let k = cal.date(from: comps) { keyed[k] = v }
        }
        // Use a thin wrapper that feeds the existing engine month-by-month with varying budgets
        // We simulate varying budgets by walking months and invoking the internal loop with the budget for that month.
        // To avoid duplicating payoff logic, we fall back to the existing function with a large horizon window while injecting extras.
        // Compute a base as the minimum of provided budgets to satisfy min-pay guardrails, and pass extras as differences.
        let values = keyed.values
        let base: Decimal = values.min() ?? 0
        var extras: [Date: Decimal] = [:]
        for (k, v) in keyed { extras[k] = max(0, v - base) }
        return try plan(
            debts: debts,
            monthlyBudget: base,
    ...
    public static func plan(
        ...
        // Wrap the original planner by varying the budget per month using extrasByMonth
        ...
        // Sum min payments
        let sumMinPayments = filteredDebts.reduce(Decimal(0)) { $0 + $1.minPayment }
        if monthlyBudget < sumMinPayments && extrasByMonth.isEmpty {
            throw DebtPlanError.infeasibleBudget(requiredMinimum: sumMinPayments)
        }
        // Normalize startDate to first of month (year-month)
        let calendar = Calendar(identifier: .gregorian)
        ...
        for monthIndex in 0..<maxMonths {
            ...
            // Step 1: Apply extra to the primary target
            if strategy != .minimumsOnly, let target = selectTargetDebt(openDebtIDs: openIDs) {
                let currentPayment = payments[target] ?? 0
                let bal = balances[target] ?? 0
                let remainingBalance = (bal - currentPayment).clampedLowerBound(0)
                let extraPayment = min(extraBudget, remainingBalance)
