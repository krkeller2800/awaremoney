...
struct FakeStatementPDFRenderer {
    ...
    private func render(recipe: SampleStatementRecipe, to outputURL: URL, fileName: String) throws -> Int {
        ...
        let pages = transactionPages(for: recipe.transactions)
    ...
    private func transactionPages(for transactions: [SampleStatementTransaction]) -> [[SampleStatementTransaction]] {
        let firstPageCapacity = 25
        let continuationCapacity = 32
    ...
    private func drawHeader(recipe: SampleStatementRecipe, pageIndex: Int, pageCount: Int) {
        ...
        drawText("Statement", in: CGRect(x: pageRect.width - margin - 170, y: pageRect.height - 82, width: 170, height: 24), attributes: .rightTitle)
    ...
    private func summaryRows(for recipe: SampleStatementRecipe) -> [(label: String, value: String)] {
        ...
        case .creditCard:
            ...
            return [
                ...
                ("Payments and Credits", currency(payments)),
                ("Purchases", currency(abs(purchases))),
        ...
        case .autoLoan, .mortgage, .genericLoan:
            ...
            return [
                ...
                ("Opening Balance", currency(recipe.openingBalance)),
                ("Recent Payments", currency(payments)),
                ("Interest and Fees", currency(abs(charges))),
...
private extension Dictionary where Key == NSAttributedString.Key, Value == Any {
    ...
    static let rightTitle: [NSAttributedString.Key: Any] = paragraph(alignment: .right, font: NSFont.boldSystemFont(ofSize: 18))
    static let sectionTitle: [NSAttributedString.Key: Any] = paragraph(alignment: .left, font: NSFont.boldSystemFont(ofSize: 13))
    static let body: [NSAttributedString.Key: Any] = paragraph(alignment: .left, font: NSFont.systemFont(ofSize: 10.5))
    static let rightBody: [NSAttributedString.Key: Any] = paragraph(alignment: .right, font: NSFont.systemFont(ofSize: 10.5))
    static let tableHeader: [NSAttributedString.Key: Any] = paragraph(alignment: .left, font: NSFont.boldSystemFont(ofSize: 9.5))
    static let rightTableHeader: [NSAttributedString.Key: Any] = paragraph(alignment: .right, font: NSFont.boldSystemFont(ofSize: 9.5))
    static let tableBody: [NSAttributedString.Key: Any] = paragraph(alignment: .left, font: NSFont.monospacedSystemFont(ofSize: 9, weight: .regular))
    static let rightTableBody: [NSAttributedString.Key: Any] = paragraph(alignment: .right, font: NSFont.monospacedDigitSystemFont(ofSize: 9, weight: .regular))
    static let footer: [NSAttributedString.Key: Any] = paragraph(alignment: .left, font: NSFont.systemFont(ofSize: 7.5), color: NSColor(calibratedWhite: 0.35, alpha: 1))
    static let rightFooter: [NSAttributedString.Key: Any] = paragraph(alignment: .right, font: NSFont.systemFont(ofSize: 7.5), color: NSColor(calibratedWhite: 0.35, alpha: 1))
    static let rightSmall: [NSAttributedString.Key: Any] = paragraph(alignment: .right, font: NSFont.systemFont(ofSize: 9))
